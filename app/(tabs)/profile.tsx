...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    // 2. 「自分宛ての友達リクエスト」を監視
    // users > (自分) > friendRequests コレクション
  ...
  const handleApprove = async (requesterId: string, requesterName: string) => {
    ...
    try {
      ...
      // B. 相手の友達リストに自分を追加（相互フォロー）
      // 自分の名前が必要なので myProfile から取得（なければ"不明"）
